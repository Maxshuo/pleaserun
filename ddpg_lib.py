...
#         self.actor_gradients = list(map(lambda x: tf.div(x, self.batch_size), self.unnormalized_actor_gradients))
        ...
        self.actor_gradients = list(map(lambda x: tf.div(x, self.batch_size), self.unnormalized_actor_gradients))
        #map(lambda x: x ** 2, [1, 2, 3, 4, 5])  # 使用 lambda 匿名函数 结果[1, 4, 9, 16, 25],map,上面梯度结果除以32
        # Optimization Op
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        # Add the action tensor in the 2nd hidden layer 添加了神经元数量
        ...
        w_init = tflearn.initializations.uniform(minval=-0.003, maxval=0.003)
        out = tflearn.fully_connected(net, 1, weights_init=w_init) #最后输出1个值
...
class GaussianNoise:
    ...
    def __call__(self):
        ...
        #GaussianNoise(),运行——call——的代码 均值为0，标准差为sigma0，大小输出一个数，高斯分布
