...
class MecTerm(object):
    ...
    """
    def __init__(self, user_config, train_config): #uer_config和train_config是字典
        ...
        if 'model' not in user_config: #如果这个模型不在user_config中，那就是markovmodel
            self.channelModel = MarkovModel(self.dis, seed=train_config['random_seed'])
        else: #在user_config中，把用户数量输入进来
            n_t = 1 #?
    ...
    def localProc(self, p): #返回的是本地计算的数据量dl，m（t）公式19
        return np.power(p/self.k, 1.0/3.0)*self.t/self.L/1000 #np，power（x，y）x的y次方 为什么除以1000？
    
    def localProcRev(self, b): #本地花费的功率，b是本地计算量
        return np.power(b*1000*self.L/self.t, 3.0)*self.k
    
    def offloadRev(self, b): #offload花费的功率  后面部分IIgmII2 公式16.和20，b是offload数据量。为什么是/呢？
    ...
    def sampleData(self): #公式17计算B（t+1）
        ...
        if self.DataBuf < 0:
            over_power = self.Power[1] - self.localProcRev(np.fmax(0, self.DataBuf+data_p)) #在本地计算还剩余的功率,多出的功率
            self.DataBuf = 0 #置buffer为0
            
        data_r = np.random.poisson(self.rate) #任务到达率服从泊松分布，泊松分布？？
        self.DataBuf += data_r #加上t时刻的任务到达 公式17
        return data_t, data_p, data_r, over_power
    
    def sampleCh(self):#获得大H
        self.Channel = self.channelModel.sampleCh()
        return self.Channel
    
    def reset(self, rate, seqCount):#重置？？？？？？
        self.rate = rate
...
#             self.DataBuf = self.data_buf_size
        ...
        sum_power = np.sum(self.Power)-over_power #总的功率消耗
...
class MecTermGD(MecTerm):
    ...
    def __init__(self, user_config, train_config, policy):
        MecTerm.__init__(self, user_config, train_config)
        self.policy = policy #         
        self.local_proc_max_bits = self.localProc(self.action_bound) # max processed bits per slot 在本地
